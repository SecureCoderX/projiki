// Updated ProjectWorkspace.jsx with Activity Timeline implementation
import React, { useState, useEffect, useMemo } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { ChangelogManager } from '../changelogs';
import ActivityTimeline from './ActivityTimeline'; // Import the new component
...
import { TaskCard, KanbanBoard, BugForm, BugCard } from '../tasks';
...
const ProjectWorkspace = () => {
  ...
  const projectTasks = useMemo(() => {
    let filtered = tasks.filter(task => task.projectId === projectId && task.type !== 'bug');
  ...
  // Get project bugs specifically
  const projectBugs = useMemo(() => {
    let filtered = tasks.filter(task => task.projectId === projectId && task.type === 'bug');
    
    if (projectSearchTerm && activeTab === 'bugs') {
      const searchLower = projectSearchTerm.toLowerCase();
      filtered = filtered.filter(bug => 
        bug.title.toLowerCase().includes(searchLower) ||
        bug.content.toLowerCase().includes(searchLower) ||
        bug.metadata?.tags?.some(tag => tag.toLowerCase().includes(searchLower))
      );
    }
  ...
  }, [changelogs, projectId, lastUpdate]);
  ...
  const projectStats = useMemo(() => {
    ...
    return {
      ...
      totalChangelogs: projectChangelogs.length
    };
  }, [projectTasks, projectNotes, projectPrompts, projectSnippets, projectBugs, projectChangelogs, lastUpdate]);
  ...
  const tabs = [
    ...
    { 
      ...
      count: projectStats.totalChangelogs 
    },
    { 
      id: 'activity', 
      label: 'Activity', 
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="p-4 border-b border-border">
            ...
            {projectSearchTerm && (
              <p className="text-sm text-text-muted mt-2">
                ...
                {activeTab === 'bugs' && `Found ${projectBugs.length} bugs`}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Other tab content remains the same - you can keep all the existing tab content here */}
          {activeTab === 'overview' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <Card className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-text-muted">Bugs</p>
                      <motion.p 
                        key={projectStats.totalBugs}
                        initial={{ scale: 1.1 }}
                        animate={{ scale: 1 }}
                        className="text-2xl font-bold text-orange-600"
                      >
                        {projectStats.totalBugs}
                      </motion.p>
                    </div>
          ...
          {activeTab === 'changelogs' && (
            <ChangelogManager projectId={projectId} />
          )}

          {/* Add all other existing tab content here - tasks, bugs, notes, prompts, snippets */}
          {/* I'll include the key ones to show the pattern */}
            ...
            <div>
              {projectBugs.length === 0 ? (
                <Card className="p-8 text-center">
                  ...
                  <h3 className="text-lg font-medium text-text-primary mb-2">
                    {projectSearchTerm ? 'No matching bugs found' : 'No bugs found'}
                  </h3>
                  <p className="text-text-muted mb-4">
                    {projectSearchTerm 
                      ? `No bugs match "${projectSearchTerm}". Try a different search term.`
                      : 'No bugs reported for this project yet. Great job!'
                    }
                  </p>
                  {!projectSearchTerm && (
                      ...
                      <BugCard 
                        ...
                        onDelete={async (bugId) => {
                          if (window.confirm('Are you sure you want to delete this bug?')) {
                            try {
                              await useTaskStore.getState().deleteTask(bugId);
                              setLastUpdate(Date.now());
                            } catch (error) {
                        ...
                        onDuplicate={async (bug) => {
                          try {
                            ...
                            delete duplicatedBug.id;
                            delete duplicatedBug.createdAt;
                            delete duplicatedBug.updatedAt;
                            
                            await useTaskStore.getState().createTask(duplicatedBug);
                            setLastUpdate(Date.now());
                          } catch (error) {
                            console.error('Failed to duplicate bug:', error);
                          }
                        }}
                      />
                    ))}
                        ...
                        onDuplicate={async (prompt) => {
                          try {
                            const duplicatedPrompt = {
                              ...
                              tags: [...(prompt.tags || []), project.name, projectId],
